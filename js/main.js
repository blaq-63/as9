...
function word_search()
{
  ...
  while(i < 15)
  {
    if(game_tiles[i].tile != "pieceX")
    {
      word = word + find_letter(game_tiles[i].tile);
      score = score + score_finder(game_tiles[i].tile);
    }
    i++;
  }
...
{
  if(game_tiles[2].tile != "pieceX")
  {
    return 1;
  }
  if(game_tiles[12].tile != "pieceX")
  {
    return 1;
  }
  //else return 0.
 ...
 */
function score_finder(given_id)
{
...
function find_tile_pos(given_id)
{
  for(var i = 0; i < 15; i++)
  {
    if(game_tiles[i].tile == given_id)
    {
      return game_tiles[i].id;
    }
  }
  return -1;
}
  ...
  for(var i = 0; i < 15; i++)
  {
    ...
    $(dropabble_tile_id).droppable({
      ...
      drop: function(event, ui)
      {
        ...
        game_tiles[find_board_pos(droppableID)].tile = dropabble_tile_id;
        word_search();
      },
        ...
        if(dropabble_tile_id != game_tiles[find_board_pos(droppableID)].tile)
        {
          console.log("Wrong Move.");
          return;
        }
        // cehck that tile was removed
        game_tiles[find_board_pos(droppableID)].tile = "pieceX";
        // word update
        word_search();
      }
    });
